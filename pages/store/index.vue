<template>
	<view class="hx-store" @touchstart="touchStart">
		 ...
		 :rightSlot="false" :background-color="[245,245,245]" :pageScroll.sync="pageScroll">
			...
			<view class="ctn">
				...
				<view class="leftBox" style="font-size: 22px;">
					<u-icon :name="shopList.collect==0?'star':'star-fill'" @click="star"></u-icon>
		...
		<!-- 头部 -->
		<view class="header">
			<image class="header-bg" :src="base+shopList.shop_image" mode="aspectFill"></image>
			...
			<view class="container storeInfo hx-shadow" :style="{height:storeInfoBoxHeight + 'px'}">
				<image class="storeAvatar hx-shadow" :src="base+shopList.logo" mode="aspectFill"></image>
		...
		<view class="main" :style="{height: mainHeight}">
			...
			<view class="tabs-box" :style="{'top': 'calc(44px + ' + statusBarHeight + 'px)','background-color':'rgba(245,245,245,'+ navSearchBgOpacity +')'}">
				<view class="" style="width: 210px; height: 100%;">
					...
					<view class="hx-tabs">
						<view class="hx-tabs-item" v-for="(item,i) in tabs" :key="i" :class="{'hx-tabs-active': swiperCurrent == i}"
							...
							<scroll-view scroll-with-animation :scroll-y="goodsBoxScroll" class="right" @scroll="asideScroll" :scroll-top="tabScrollTop">
								<view class="goodsListBox">
									...
									<view class="category" v-for="item in categoryList" :key="item.id" :id="'goodsBox'+item.id">
										...
										<view class="list">
											<view class="box" v-for="(rowData,i) in item.goods" :key="rowData.id">
												<!-- 商品列表 -->
												<!-- <m-store-pro @touchOnGoods="touchOnGoods" :rowData="box"></m-store-pro>
													 -->
												<!-- 商品列表 -->
												<view class="m-store-item">
													<view class="m-img" @click="hrefGoodsInfo(rowData)">
														<image style="width: 100%;height: 100%;" :src="base+rowData.images.split(',')[0]" mode="aspectFit"></image>
													</view>
													<view class="m-text">
														<view class="m-title" @click="hrefGoodsInfo(rowData)">
															{{rowData.name}}
														</view>
														...
														<block v-else>
															<view class="m-price-box">
																...
																<view class="m-old-price" v-if="rowData.line_price">
																	<text>￥{{rowData.line_price}}</text>
																</view>
															</view>
														</block>
														<view class="m-distance">

															<block v-if="rowData.goods_spec">
				...
				<swiper-item class="swiper-item">
					<scroll-view scroll-y style="height: 100%;width: 100%;background-color: #ffffff;" @scroll="asideScroll" :scroll-y="goodsBoxScroll">
						<view class="scroll-items evaluate-box">
							...
							<hx-comment :listData="commentList" :totalGrade="totalGrade"></hx-comment>
						</view>
					</scroll-view>
				</swiper-item>
					...
					<view class="scroll-items business-box">
						...
						<view class="info-list hx-mt-15 ">
							<view class="info-list-container hx-bb">
								...
								<text>配送时间：{{ shopList.give_time_text }}</text>
							</view>
						</view>
						<view class=" info-list">
							<view class="info-list-container" @click="goCall(storeData.telephone)">
								<i class="hxicon-phone"></i>
								<text style="flex: 1;">商家电话：{{ shopList.phone }}</text>
								<view class="right" @click="goCall(shopList.phone)">
		...
		<view class="foot" @touchmove.stop.prevent="mpClear" :style="{height: footHeight}" v-if="showFoot">
			...
			<view class="btn-box">
				<view class="btn-box-left" @click="goCall(shopList.phone)">
					<view class="imgBox">
				...
				<view class="btn-box-center" @click="showShoppingCar">
					<view class="cart" :animation="cartAnimationData">
						<view class="tag cartNum" v-if="shoppCart.total_num>0">{{shoppCart.total_num}}</view>
						<image :src="goodsTotalNumber ? '/static/store/cart.png' : '/static/store/cart2.png'" mode=""></image>
					</view>
					<view class="priceBox">
						<view class="hx-txt-18 hx-color-white" v-if="shoppCart.total_prices>0">
							￥{{shoppCart.total_prices}}
						</view>
						<view class="hx-txt-10 hx-color-gray">
							另需配送费￥{{shoppCart.shipping_free}}
						</view>
					</view>
				</view>
				<view class="btn-box-right">
					<view class="jiesuan" v-if="shoppCart.total_prices>0 && shoppCart.close==1" @click="jiesuan">
						去结算
					</view>
					...
					<view class="" style="flex: 1;">
						<scroll-view scroll-y="true" class="goods-list-scroll" :scroll-top="carGoodsScrollTop">
							<view class="goods-list">
								<view class="box" v-for="(rowData,i) in shoppCart.list" :key="rowData.id">

									<view class="m-store-item">
										<view class="m-img">
											<image style="width: 100%;height: 100%;" :src="base+rowData.goods_image" mode="aspectFit"></image>
										</view>
										<view class="m-text">
											<view class="m-title">
												{{rowData.goods_name}}
											...
											<view class="m-descripe">
												...
												{{rowData.goods_spec_name}}
											</view>
											<view class="m-price-box">
												<view class="symbol">￥</view>
												<view class="m-price">{{rowData.goods_price}}</view>
												<view class="m-old-price" v-if="rowData.goods_line_price">
													<text>￥{{rowData.goods_line_price}}</text>
												</view>
											</view>
											<view class="m-distance">
												<view :class="'addEle2_' + rowData.id" class="jumpPosition">
												</view>
												<u-number-box disabled-input v-model="rowData.goods_num" @minus="cartDelgoods(rowData.goods_id,rowData.goods_spec_name)"
												 @plus="cartAddgoods(rowData.goods_id,rowData.goods_spec_name)"></u-number-box>
		...
		<uni-popup ref="popup" type="center" @change="popupChange">
			<view class="form-main" v-if="currentGoodsData.name">
				<view class="form-main_ctn" @click.stop.prevent="mpClear">
					...
					<view class="gg_box">
						<block v-for="form_child,index in currentGoodsData.goods_spec" :key="form_child.id">
							<view class="item" :class="{'active': form_child.select}" @click="selectGoodsForm(form_child,index)">{{form_child.name}}</view>
						</block>
					</view>
					...
					<view class="select_gg">
						...
						<view class="select_gg_box">
							<block v-for="form_child in currentGoodsData.goods_spec" :key="form_child.id" v-if="form_child.select == true">
								<view class="gg-item">
		...
		methods: {
			...
			star() {
				this.$u.get('/api/shop/collect', {
					...
					if (res.data == 1) {
						this.shopList.collect = 1
					} else if (res.data == 2) {
						this.shopList.collect = 0
					}
				})
			},
			// 购物车删除商品
			...
			selectGoodsForm(formChild, indexKey) {
				...
				this.currentGoodsData.goods_spec.forEach((item, index) => {
					if (indexKey == index) {
						...
						this.currentGoodsData.goods_spec[index].select = false
					}
