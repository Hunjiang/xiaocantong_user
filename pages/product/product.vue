	...
	<view class="container" id="productPage">
			...
			<view class="ctn">
				<view class="tabs-box" style="background: none; width: auto;" :style="!showRing ? '' : 'display:none;'">
					<view class="" style="width: 210px;height: 100%;">
						<view class="hx-tabs">
							<view class="hx-tabs-item" style="justify-content: center;" v-for="(item,i) in tabs" :key="i" :class="{'hx-tabs-active': swiperCurrent == i}" @click="swiperChange(i,item.id)" >
								<text>{{item.name}}</text>
							</view>
							<view class="hx-tabs-slider-box" style="justify-content: center;" :style="{left:swiperCurrentSliderLeft + 'px'}">
		...
		<view class="carousel">
			<swiper indicator-dots circular=true duration="400">
				<swiper-item class="swiper-item" v-for="(item,index) in swiperList" :key="index" @click="previewImg(item,swiperList)">
...
<!-- 			<view class="tag-box">
			...
			<!-- </view> -->
			<view class="bot-row">
				<text>月售{{ goodsInfoData.sales ? goodsInfoData.sales : 0 }}</text>
				<!-- <text>库存: 4690</text>
				<text>浏览量: 768</text> -->
			</view>
		...
		<view class="container-box shopping-cart" :class="{'barBorder':showBorder}">
			<view class="price-box" >
				...
				<block v-if="goodsInfoData.line_price != 0 && goodsInfoData.line_price > goodsInfoData.price">
					<text class="m-price" >¥{{ goodsInfoData.line_price }}</text>
...
<!-- 			<block v-else>
		...
		<view class="container-box">
			...
			<view class="info-box" id="detail">
				<view class="info-item" >
					<text class="left">菜品描述：</text>
					<text class="right">{{goodsInfoData.depict}}</text>
				</view>
				<view class="info-item">
					<text class="left">菜品主料：</text>
					<text class="right">{{goodsInfoData.ingredient}}</text>
				</view>
				<view class="info-item">
					<text class="left">菜品口味：</text>
					<text class="right">{{goodsInfoData.taste}}</text>
				</view>
				<view class="info-item">
					<text class="left">菜品备注：</text>
					<text class="right">{{goodsInfoData.note}}</text>
				</view>
			</view>
		...
		<view class="container-box eva-section" id="eva">
			...
			<!-- </view> -->
			<!-- 最好不要超过两条评价 -->
			<view class="eva-box" v-for="(item, i) in goodsEvaData.list" :key="i">
				<image class="portrait" :src="base+item.avatar" mode="aspectFill"></image>
				<view class="right">
					<view class="bot">
						<text class="attr">{{item.nickname}}</text>
						<text class="time">{{item.create_time_text}}</text>
					</view>
					<text class="con">{{item.content}}</text>
					<view class="imgs" v-if="item.images">
						<block v-for="(imgurl, index) in getImg(item.images)" :key="index">
							<view class="imgs-box"><image :src="imgurl" mode="widthFix" style="width: 100%;" @click="previewImg(imgurl,getImg(item.images))"></image></view>
						</block>
					</view>
				</view>
...
<script>
	...
	export default{
		...
		async onLoad(options){
			...
			let goodsDetail = JSON.parse(decodeURIComponent(options.item))
			...
			setTimeout(()=>{
				...
				that.swiperList=that.swiperList.map((item,index)=>{
					console.log(this.base+item)
					return this.base + item
				})
				//商品评价
				this.$u.get('/api/goods/goodsComment',{
