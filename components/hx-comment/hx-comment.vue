		...
		<view class="hx-comment_basic-info">
			<view class="hx-comment_basic-info_left">
				<view class="hx-comment_basic-info_left_top">
					<text>{{totalGrade.total_grade}}</text>
				</view>
			...
			<view class="hx-comment_basic-info_center">
				<view class="hx-comment_basic-info_center_item" style="margin-bottom: 10px;">
					...
					<view class="hx-comment_basic-info_center_item_center">
						<view>{{totalGrade.goods_num}}</view>
				...
				<block v-for="(item, index_) in listData" :key="index_">
					<view class="item">
						...
						<view class="info">  
							...
							<view class="stars">
								<text>评分</text>
								<u-rate :count="5" v-model="item.rate" size="26" active-color="#f6d200" :disabled="true"></u-rate>
							</view>
							<view class="evaluate-content">
								<text>{{ item.content || '用户暂未评价' }}</text>
								<view class="imgs" v-if="item.images">
									<block v-for="(imgurl, index) in item.images" :key="index">
										<view class="imgs-box"><image :src="imgurl" mode="widthFix" style="width: 100%;" @click="previewImgs(imgurl,item.images)"></image></view>
									</block>
								</view>
							</view>
						</view>
